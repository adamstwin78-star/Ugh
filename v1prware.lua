...
local function clearTag(tag)
    ...
    -- For puddles the holder lives in workspace, clean those up too
    if tag == "puddle" then
...
local function addPuddleHighlight(part, color, tag, label)
    ...
    -- Highlight on the part itself (works fine parented to part)
    local hl = Instance.new("Highlight")
    ...
    task.defer(function()
        ...
        -- Parent holder to workspace so CylinderHandleAdornment renders correctly
        -- Use a RenderStepped connection to keep it snapped to the part position
        ...
        holder.Parent      = svc.WS  -- workspace, not part
        ...
        -- Keep holder snapped to part every frame
        local followConn = svc.Run.Heartbeat:Connect(function()
            if not part.Parent then return end
            holder.CFrame = CFrame.new(part.Position + Vector3.new(0, 0.05, 0))
        end)

        -- Resize circles when puddle grows
        ...
        -- Cleanup when puddle is removed
...
local function isJohnDoePuddle(obj)
    ...
    -- Match "Shadows", "JohnDoeShadows", "ShadowsFolder", etc.
...
do
    ...
    local jd_AXE_PREDICTION = 0.15 -- seconds to predict killer position ahead
    ...
    local function jd_axeStartLock()
        ...
        jd_axeLockConn=jd_Run.Heartbeat:Connect(function()
            ...
            -- Predict where the killer will be jd_AXE_PREDICTION seconds from now
    ...
    jdAxe:Slider({Title="Prediction (s)", Desc="How far ahead to lead the killer position", Step=0.01,
        Value={Min=0.0,Max=1.0,Default=jd_AXE_PREDICTION},
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabNoli = win:Tab({ Title = "Noli", Icon = "zap" })

------------------------------------------------------------------------
-- HELI-TECH
------------------------------------------------------------------------
local secHeli = tabNoli:Section({ Title = "Heli-Tech Macro (2nd Void Rush)", Opened = true })
...
secHeli:Toggle({ Title="Enable Heli-Tech Macro", Type="Checkbox", Default=false,
    Callback=function(on) heli.enabled=on end })
secHeli:Slider({ Title="Launch Strength", Desc="Higher = more air distance", Step=1,
    Value={Min=50,Max=160,Default=heli.strength},
    Callback=function(v) heli.strength=v; cfg.set("heliStrength",v) end })
secHeli:Slider({ Title="Hold Time (s)", Desc="How long to hold the boost", Step=0.01,
    Value={Min=0.15,Max=0.65,Default=heli.holdTime},
    Callback=function(v) heli.holdTime=v; cfg.set("heliHold",v) end })
secHeli:Button({ Title="Test Heli-Tech Now (H key)", Callback=function() heliLaunch() end })

------------------------------------------------------------------------
-- NOVA SILENT AIM
-- Always returns the solved ballistic CFrame from OnClientInvoke
-- regardless of where you're currently looking — no facing check at all.
------------------------------------------------------------------------
...
local function novaGetNearest(fromPos)
    -- Try the standard Players/Survivors folder first
    local pf = svc.WS:FindFirstChild("Players")
    local sf = pf and pf:FindFirstChild("Survivors")
    -- Fallback: sometimes the folder sits directly under workspace
    if not sf then sf = svc.WS:FindFirstChild("Survivors") end
...
local function novaBuildCF(myHRP, targetHRP)
    ...
    -- Use sampled velocity for smoother prediction than AssemblyLinearVelocity alone
...
local function novaApplyPatch()
    ...
    nova.networkRF.OnClientInvoke = function(reqName, ...)
        -- Always intercept GetCameraCF regardless of where camera is pointing.
        -- No facing check — silent aim works from any angle.
...
-- Re-apply patch on respawn since the character reference changes
lp.CharacterAdded:Connect(function()
...
secNova:Toggle({ Title="Enable Nova Silent Aim", Type="Checkbox", Default=false,
    Callback=function(on)
        nova.enabled = on
        if on then novaApplyPatch() else novaRemovePatch() end
    end })
secNova:Slider({ Title="Prediction", Desc="Higher = better lead on moving targets", Step=0.01,
    Value={Min=0, Max=2, Default=nova.prediction},
    Callback=function(v) nova.prediction=v; cfg.set("novaPrediction",v) end })
secNova:Slider({ Title="Aim Offset Y", Desc="Vertical adjustment", Step=0.1,
    Value={Min=-5, Max=5, Default=nova.aimOffsetY},
